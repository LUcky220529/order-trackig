...
export default function OfflineBillModal({ isOpen, onClose, onSuccess }: OfflineBillModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (form.express) hours = Math.max(12, hours / 2);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      {lastOrderId ? (
        ...
        <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-8 text-center animate-in fade-in zoom-in duration-300">
          <div className="w-20 h-20 rounded-full bg-gradient-to-br from-[#b8963e] to-[#d4af5a] flex items-center justify-center mx-auto mb-6 text-white text-4xl shadow-lg">✓</div>
          ...
          <div className="space-y-3">
            <a
              ...
              className="w-full flex items-center justify-center gap-2 bg-[#0a1628] text-[#d4af5a] py-3.5 rounded-xl font-bold hover:scale-[1.02] transition-all shadow-md"
            >
              📦 View Tracking Page
            </a>
            <button
        ...
        /* Form Screen */
        <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
            ...
            <div className="space-y-6">
              ...
              <div>
                <label className="flex items-center gap-2 cursor-pointer group">
                    ...
                    <span className="text-sm font-bold text-[#0a1628] group-hover:text-[#b8963e] transition-colors">Express Service (50% extra, faster delivery)</span>
                </label>
              </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-3">
                  ...
                  <button onClick={addItem} className="text-xs bg-[#e8edf3] hover:bg-[#d4af5a] hover:text-[#0a1628] text-[#4a5568] px-3 py-1.5 rounded-lg font-bold transition-all">+ Add Item</button>
                </div>
          ...
          <div className="p-6 border-t border-[#e8edf3] bg-[#f8fafc] flex justify-between items-center">
            ...
            <button onClick={handleSubmit} disabled={loading} className="bg-gradient-to-r from-[#0a1628] to-[#1e3a5f] hover:from-[#b8963e] hover:to-[#d4af5a] text-white font-bold py-3.5 px-8 rounded-xl shadow-lg transition-all disabled:opacity-50">
